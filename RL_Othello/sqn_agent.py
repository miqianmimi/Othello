...
class RL_QG_agent:
    def __init__(self):
        ...
        self.init_model()
        self.load_model(self.sessI,I=270)


    def init_model(self):
        self.sessI=tf.Session()
    ...
    def place(self,state,enable,player):
            ...
            if player==1:
                predict = self.sessI.run(self.predict_white, feed_dict={self.inputs1: input1,self.accept:accept.reshape([1,-1])})[0]
            else:
                predict = self.sessI.run(self.predict_black, feed_dict={self.inputs1: input1,self.accept:accept.reshape([1,-1])})[0]
    ...
    def save_model(self,sess,I):
        ...
        self.saver.save(sess, os.path.join(self.model_dir, 'parameter'+'MODEL'+str(I)+'.ckpt'))
        return (sess)
    ...
    def load_model(self,sess,I):
        ...
        # 重新导入模型
        self.saver = tf.train.Saver()
        self.saver.restore(sess, os.path.join(self.model_dir, 'parameter'+'MODEL'+str(I)+'.ckpt'))
