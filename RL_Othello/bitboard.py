...
def board_to_bit(state):
    black_list = list(state[0].flatten())
    white_list = list(state[1].flatten())
    black = '0b' + ''.join([str(int(item)) for item in reversed(black_list)])
    white = '0b' + ''.join([str(int(item)) for item in reversed(white_list)])
    return int(black, 2), int(white, 2)
...
def find_correct_moves(own, enemy):
    ...
    l = list(bin(mobility))[2:]
    mobility = list(len(l) - 1 - np.where(np.array(l) == '1')[0])
    if len(mobility) == 0:
        mobility = [65]
    return mobility
...
def step(pos, own, enemy):
    pos = int(pos)
    if pos == 65:
        return own, enemy
    flip = calc_flip(pos, own, enemy)
    try:
        new_own = (1 << pos) + own + flip
    except TypeError:
        import pdb;pdb.set_trace()
    new_enemy = enemy - flip
    return new_own, new_enemy
...
def is_finished(black, white):
    if 65 in find_correct_moves(black, white) and 65 in find_correct_moves(white, black):
        return True
    return False
...
def bit_to_board(black, white):
    ...
    for i in range(64):
        ...
        black >>= 1
        white >>= 1
    return np.array([np.array(black_array).reshape([8, 8]), np.array(white_array).reshape([8, 8])])
